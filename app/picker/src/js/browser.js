...
function openMapApp(lat, lng, addr) {
    ...
    if (ua.android) {
        alert('android');
        // 百度地图uri api
        uri[0] = "bdapp://map/navi?location=" + lat + "," + lng + "&query=" + addr;
        // 高德地图uri api
        ...
        alert('IOS');
        // 百度地图uri api
        uri[0] = "baidumap://map/navi?location=" + lat + "," + lng + "&query=" + addr;
        // 高德地图uri api
        uri[1] = "iosamap://navi?sourceApplication=xlwx&poiname=" + addr + "&lat=" + lat + "&lon=" + lng + "&dev=1&style=2";
        // 腾讯地图uri api
        uri[2] = "qqmap://map/marker?marker=coord:" + lat + "," + lng + ";title:" + addr + "&referer=xlwx";
        // 苹果地图uri api
        uri[3] = "http://maps.apple.com/?sll=" + lat + "," + lng + "&address=" + addr;
    }
    // //调用uri
    if(uri.length == 0){
        return;
