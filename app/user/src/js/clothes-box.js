$(function(){
            ...
            mui.confirm("确定删除选中的商品？","提醒",["否","是"],function(e){
                if(e.index==1){
                    ...
                    for(var i=0;i<$("form li").length;i++){
                        if($("form li").eq(i).find($("input[type=checkbox]")).is
                            (":checked")){
                            ...
                            for(var j=0;j<a;j++){
                                var partsId=$("form li").eq(i).find
                                ($(".tail-mes span")).eq(j).attr("id");
    ...
    //减少商品
    $("#shop-car").on("tap",".btn-reduce",function(){
        if($(this).next().val()<=1){
            mui.alert("商品数量不能小于1");
        }else{
            var o=$(this);
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    for(var j=0;j<$("form li").length;j++){
                        var even=parseFloat($("form li").eq(j).find
                        ($(".nowPrice")).html());
                        sum+=even;
                    }
    ...
    //增加商品
    $("#shop-car").on("tap",".btn-add",function(){
        var o=$(this);
        var _id=o.parent().parent().parent().attr("data-id");
        var thisMes=o.parent().parent().parent().find($(".tail-mes span"));
        var arr=[];
        ...
        if($("form li").length<1){
            ...
            for(var i=0;i<$("form li").length;i++){
                ...
                if($("form li").eq(i).find($("input[type=checkbox]")).is(":checked")){
                    ...
                    for(var j=0;j<a;j++){
                        var partsId=$("form li").eq(i).find($(".tail-mes span")).eq
                        (j).attr("id");
                        partsIds.push(partsId);
                    }
...
function ajaxInit(){
    ...
    enAjax(url,'get',true,data,getGoods,null,true);
    function getGoods(data){
        ...
        for(var j=0;j<data.length;j++){
            ...
            var item='<li data-id="'+data[j].goodsId+'" num="'+data[j].price+'">'+
                ...
                '<div class="mui-numbox" data-numbox-min="1" style="display: flex">'+
                '<button class="mui-btn btn-num btn-reduce" type="button">-</button>'+
            '<input disabled class="mui-input-numbox" type="number" data-numbox-min="1" value="'+data[j].commodityCount+'">'+
            '<button class="mui-btn btn-num btn-add" type="button">+</button>'+
            '</div>'+
            '</div>'+
            '</li>';
