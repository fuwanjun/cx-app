...
$(function(){
    ...
    $(".vip-con").on("tap", ".adds", function () {
        ...
        if(_id==11){
			window.location.href="order-pay.html?goodsId="+_id;
        }else{
            var left=$(this).offset().left;
            var top=$(this).offset().top;
            var carTop=$("#shop-car").offset().top;
            var carLeft=$("#shop-car").offset().left;
            var join='<div style="width: 0.25rem;height:0.25rem;position:absolute;top:'+top+'px;left:'+left+'px;border: 1px solid #39a4f2;background: #39a4f2;border-radius: 50%;"><img style="width: 100%;" src="../../../public/img/picker/add.png" alt=""></div>';
...
function getItem(type){
	$.ajax({
		...
		success:function(data){
//			console.log(data)
			$(".vip-con").eq(type-5).html("");
			var item='';
			for(var i=0;i<data.length;i++){
				var img=data[i].goodsPicture?"http://"+data[i].goodsPicture:'../../img/shose.jpg';
